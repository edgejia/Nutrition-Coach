...
5. 若是單一食材餐點，使用者只要求調整單一欄位（例如只改蛋白質）時，可以保留其他欄位不變，只更新該欄位。
6. 若使用者已明確授權你自行估一個合理數字（例如「正常平均幾g就幾g」），就先決定一個具體數字，再直接套用；不要再回報格式錯誤或要求同一句提供完整整筆欄位。
7. 成功修改歷史餐點時，要明確表示是更新原本那筆紀錄，不是新增一筆。成功刪除時，要明確表示已刪除原本那筆餐點。`);
