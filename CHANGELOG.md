...
- 修補 issue #134 新揭露的 runtime dependency advisories：將可由圖片上傳路徑觸及的 `sharp` 升至 `0.35.3`，並將 Fastify runtime 鏈上的 `fast-uri` 與 `brace-expansion` security resolutions 分別升至 `3.1.4`、`5.0.7`；當時既有的 Drizzle 與 `form-data` defer 依 ADR 0009 保留，Drizzle 隨後由下列 0.45.2 升級解除。
