...
1. The source PR is merged into `main` by maintainer decision.
2. Post-merge local planning archive is complete from updated `main` when the GSD workflow is active. An active GSD maintenance pause blocks this step rather than allowing it to be skipped.
3. A fresh runtime-refresh decision selects the exact merged source SHA.
4. **B01** receives fresh approval for a bounded runtime quiescence and storage backup.
5. The backup is independently verified and remains current while writes stay quiesced.
6. **R05** receives separate fresh approval for `yarn db:migrate`.
7. **R06** receives separate fresh approval for build/start or restart.
8. Tunnel changes and public smoke remain separate later approvals.
