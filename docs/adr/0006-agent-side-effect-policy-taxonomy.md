...
|---|---|---|---|
| `delete_meal` | `direct-execute` | delete_meal_requires_resolved_target: Direct delete requires a same-turn resolved target; no delete proposal path exists in Phase 85.<br>delete_meal_revision_precondition_guard: Direct delete requires the resolved target revision to remain current. | Named rules: delete_meal_requires_resolved_target (blocked), delete_meal_revision_precondition_guard (blocked) |
