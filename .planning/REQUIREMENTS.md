...
- [ ] **INTENT-01**: User-explicit meal period intent such as `早餐`, `午餐`, `晚餐`, or `宵夜` is persisted as the meal period authority for new logs, even when the current clock hour would infer a different period.
- [ ] **INTENT-02**: Current-day and historical meal rows expose period information from persisted structured facts instead of deriving display period only from `loggedAt` hour.
