...
## v2.6 Meal Editing and History Usability (Shipped: 2026-06-03)
...
## v2.5 Structured LLM Boundaries and DTO Reliability (Shipped: 2026-06-02)

**Phases completed:** 5 phases, 15 plans, 34 tasks

**Key accomplishments:**

- Provider-level structured JSON output with validator-owned trust, metadata-only failures, OpenAI runtime support, and deterministic mock parity.
- Onboarding target generation now uses strict structured object output with service-owned Zod validation, retry/fallback classification, and sanitized telemetry.
- `/api/device` now has route-level structured-output proof for generated targets, deterministic fallback persistence, unchanged response shape, and sanitized target-generation telemetry.
- Shared client DTO predicates now protect EventSource push frames and direct Zustand authoritative writes without adding a second client state boundary.
- Client API transport now fails malformed REST authority closed, omits malformed terminal stream additions, and blocks incomplete history rows from editable meal payloads.
- Fastify/SQLite route proof now verifies goals, day snapshot, history meal, and history trend responses as public DTO projections without trusting raw server output.
- Failing Wave 0 unit contracts for atomic receipt/outcome persistence and structured mutation history authority
- Failing Fastify/SQLite route contracts for fail-closed receipt projection after committed meal mutations
- Additive `chat_mutation_outcomes` storage with strict validated fact formatting for future compressed-history authority
- Atomic assistant receipt/outcome persistence with validated structured compressed-history facts
- Committed orchestrator mutation effects now produce safe structured facts for receipt persistence and compressed-history authority
- JSON/SSE chat finalization now exposes receipt identity only after atomic assistant, receipt, and structured outcome persistence succeeds
- Deployed-like guest-session boot validation plus explicit local-only Fastify CORS policy with focused unit and integration proof.
- Shared route fallback catch-field redaction for structured events and llm-trace.v2, with unit and real-route proof.
- v2.5 local release proof now links structured-boundary, DTO, receipt/history, and release-security evidence to green TypeScript and release gates without promotion.
