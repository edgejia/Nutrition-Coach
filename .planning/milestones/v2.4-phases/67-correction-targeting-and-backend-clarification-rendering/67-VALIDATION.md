...
- [x] `tests/unit/meal-correction.test.ts` - red-first cases for explicit period over inferred period, period plus `那餐` with newer non-matching meal, invalid number re-show, label-set clarification ordering, unmatched food label no-fallback behavior, and D-30 clear single-date no-safe-target recovery: same-date meals -> numbered list, no meals -> no-meals copy, no cross-date nearest candidates.
- [x] `tests/unit/orchestrator.test.ts` - red-first renderer-owned terminal clarification case where the LLM attempts to paraphrase or echo raw target text after `find_meals`.
- [x] `tests/unit/tools.test.ts` - red-first controlled `find_meals` result proof if the renderer contract is added at the tool boundary.
- [x] `tests/integration/chat-meal-correction.integration.test.ts` - red-first Fastify proof for stable numbered backend copy, no mutation, no `summaryOutcome`, no publish, and no raw correction echo.
...
- [x] Feedback latency <30s for task-level single-file checks; broad gates are reserved for wave/final checks.
- [x] `nyquist_compliant: true` set in frontmatter after Wave 0 proof is in place.
